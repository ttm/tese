...
{\footnotesize Em uma rede livre de escala, a probabilidade $p(k_i)$ de escolher ao acaso um vértice de grau $k_i$ é:
	$ p(k_i)=C.k_i^{-\alpha}$, em geral $1,5<\alpha<3$.}
\begin{figure}
	\begin{columns}
		\column{.1\textwidth}
...
$N$ e $\overline{k}$ são sempre diretamente proporcionais à quantidade de recursos alocados se fixado um deles ($N$ ou $\overline{k}$).
...
\begin{itemize}
	\item a teoria básica de complexidade estabelece: estes sistemas são adaptativos para eficiência e sobrevivência, processam informação, interagem entre si, por vezes se replicam (e.g. idosos em uma praça)
...
\frametitle{- teste de Kolmogorov-Smirnov para incidencias em textos}
...
preciso e eficiente dos desenvolvimentos. Mais especificamente:
	\begin{itemize}
		...
		\item {\bf \color{red} streaming de estruturas sociais}
		\item {\bf \color{red} sistema de monitoramento massivo e aberto}
	\end{itemize}
\end{frame}
