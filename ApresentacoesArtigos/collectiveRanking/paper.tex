...
\title{Approval and participation indexes for a voting process} % For titles, only capitalize the first letter
...
\begin{abstract}
In finding the adequate way to prioritize proposals, the Brazilian participation community agreed about an approval index and a participation index.
...
Beyond innovation, the relevance of this report relies in the nearby use of these indexes to raise and prioritize proposals about public health care in open processes and assist public managers in federal initiatives.
\end{abstract}
...
\end{split}
\end{equation}
where $v^+_i$, $v^-_i$ and $\eta_i$ are approval count, disapproval count and exhibition count, respectively. Note that $\eta_i-v^+_i-v^-_i$ is the count of the ``indifferent'' manifestations received by proposal $i$. Also, note that accepted sampling estimates error, such indexes are expected to be constant above a minimum threshold.
