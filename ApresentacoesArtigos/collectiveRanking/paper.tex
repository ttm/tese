...
Both practice and literature is constantly handled by the experts involved, and the formalization of such model and indexes seems novel.
Also, the relevance of this report is strengthened by the nearby use of these indexes, by the Brazilian General Secretariat of the Republic, to raise and prioritize proposals about public health care in open processes and assist federal public managers.
\end{abstract}
...
the processes themselves vary, and the fact that the ranking indexes presented here seem not to be
...
Some of which are:
\begin{itemize}
	\item adequate estimates of threshold for statistical validity of the ranking.
    \item Keeping the IP of voters to ease detection of automated and other fraudulent efforts.
    \item The use of the outcomes from the voting process. This requires probing the survey being conducted and its purposes. The indexes here presented targets the deliverance of indicatives for the Brazilian federal government about the most important health care proposals. Given the unauthenticated voting, the outcomes might be regarded as a reference rankings if data is minimally shared and checked for inadequate data entry (such as voting by automated scripts or one persistent participant).
\end{itemize}
...
\end{split}
\end{equation}
where $v^+_i$, $v^-_i$ and $\eta_i$ are approval count, disapproval count and exhibition count, respectively. Note that $\eta_i-v^+_i-v^-_i$ is the count of the ``indifferent'' manifestations received by proposal $i$. Also, such $\alpha_i$ and $\gamma_i$ indexes are expected to be constants for each proposal $i$, plus a sampling estimate error that should be smaller as $\eta_i$ raises. This error is thought to be acceptable if $\eta_i$ is above a threshold established by the participation community and public managers.
...
A Brazilian solution, used in diverse software and specially important as the output of a nation-wide decision making need, is the Agora Algorithm~\cite{agora}. It presents a decision procedure in phases (agenda proposition, deliberations proposition and commenting, voting) based on agendas with posting and voting and posting types (comment or proposal, the last can become a resolution).
Although coherent, this framework requires authentication and might need experimentation and tuning so to achieve the desired use with more than dozens or a few hundreds of participants.
